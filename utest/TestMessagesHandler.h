...
TEST_F(TestMessagesHandler, ThrowOnInvalidInput)
{
    ...
    EXPECT_CALL(*mockStorage, Store(_)).Times(0);
    EXPECT_CALL(*mockStorage, Read(_)).Times(0);
    EXPECT_CALL(*mockStorage, Update(_)).Times(0);
    EXPECT_CALL(*mockStorage, Remove(_)).Times(0);
    EXPECT_CALL(*mockStorage, Authenticate(_)).Times(0);
...
TEST_F(TestMessagesHandler, AuthenticateMessageProcessedCorrectly)
{
    ...
    EXPECT_CALL(*mockStorage, Update(_)).Times(0);
    EXPECT_CALL(*mockStorage, Remove(_)).Times(0);
    EXPECT_CALL(*mockStorage, Authenticate(_)).Times(1).WillOnce(Return(true));
...
{
    std::shared_ptr<MockdataStorageHandler> mockStorage = std::shared_ptr<MockdataStorageHandler>();
    EXPECT_CALL(*mockStorage, Store(_)).Times(1);
    EXPECT_CALL(*mockStorage, Read(_)).Times(0);
    EXPECT_CALL(*mockStorage, Update(_)).Times(0);
    EXPECT_CALL(*mockStorage, Remove(_)).Times(0);
    EXPECT_CALL(*mockStorage, Authenticate(_)).Times(0);
